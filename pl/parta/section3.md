...
有时需要在柯里化函数、非柯里化函数之间进行转换，有时需要改变柯里化化函数的 **参数顺序**，在 ML 中，这些都可以轻易实现：
...
```
* `curry range` 将返回一个柯里化函数；

## 回调函数

### Mutable Reference

reference 是 ML 中唯一可修改的东西。

引用类型：`t ref`，其中 `t` 是类型。

基本表达式：

1. 创建引用：`ref e` 使用 `e` 作为初始内容，创建引用；
2. 更新引用：`e1 := e2`
3. 获取内容：`!e`
...
val w = (!y) + (!z); (* 85 *)
```
* 当不关心表达式的 **计算结果**，仅需要其 **副作用** 时，使用 `val _ = e`；

### 实现回调函数

回调函数也是闭包的惯用法之一：一个库，接受客户端注册，并保存客户端的函数，当事件发生时，执行客户端提供的函数，这些函数被称为回调函数。
...
* `signature x = List;` 查询 `List` 的所有函数；

## Abstract Data Type

Abstract Data Type 也是闭包的惯用法之一，但是一般不会这样用，因为现代语言一般有 **更简洁** 的方式完成相同效果。

ML 中通过 `a record of closure` 来实现 Abstract Data Type，这与面向对象中的 **类** 很类似：

* ADT 中的函数（闭包）对应 OOP 中的公共方法；
* ADT 中闭包的 `environment` 中的绑定类似 OOP 中的私有字段、私有方法；

通过 ADT 暴露的闭包来使用 ADT，从而可以随时修改 ADT 的内部实现，这与 OOP 中类的作用很类似。
